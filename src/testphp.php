...
<body class="relative bg-[url('resources/lib-bg.jpg')] h-screen text-cabin text-white">
    <div class="fixed inset-0 bg-black/50 h-screen z-0"></div>
    <header class="group fixed top-0 pt-10 pb-10 w-20 hover:w-60 duration-500 ease-out h-screen flex flex-col justify-between bg-[#060d0d99] backdrop-blur-md shadow-[var(--around-shadow-md)] select-none z-10">
        ...
        <menu class="flex flex-col gap-2">
            ...
            <li><a href="admin.html" class="flex items-center gap-8 pl-5 py-2 hover:opacity-60 duration-200 ease-linear">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="min-w-8 w-8">
    ...
    <!-- ================================================== MAIN ================================================== -->
    <main class="relative ml-25 m-5 p-15 w-[calc(100vw-135px)] min-h-[calc(100vh-40px)] h-auto rounded-4xl bg-[#eeeeee] z-2 text-black drag-none">
        <h1 class="mb-5 text-3xl font-bold text-[#585345] select-none">Library</h1>
        <div class="absolute top-15 right-15 flex flex-row-reverse gap-5 items-center text-dirty-brown font-bold text-lg select-none">
            <a href="profile.html"><img src="resources/dp/profilepic.jpg" alt="dp1" class="size-12 rounded-full border-2 border-dirty-brown cursor-pointer" draggable="false"></a>
            <p>Welcome User!</p>
        </div>
        <div class="flex justify-between items-center">
            <form action="" id="search-form" class="flex items-center gap-5">
                <span id="srch">
                    ...
                    <input type="search" name="search" id="search" placeholder="Search for a thesis book..." class="px-5 py-1 w-80 border-2 rounded-2xl border-[#585345] text-[#585345] outline-none">
                </span>
        ...
        <div id="all-ths" class="text-gray-100">
            ...
            ?>
                <div class="relative w-110 h-45 libitems" id="<?php echo $thesis_id;?>" > <!-- THESIS_ID to be placed here (id="") -->
                    <div class="absolute w-full h-full flex ">
                        ...
                        <div class="w-35 h-full select-none z-3">
                            <img src="resources/book.svg" alt="book" onclick="toggleFullView(<?php echo $thesis_id;?>)" class="absolute top-2.5 ml-2.5 w-30 hover:-translate-y-2 duration-200 cursor-zoom-in" draggable="false">
                        </div>
                        <div class="pt-8 w-70 flex flex-col gap-2 justify-start z-3">
                            <!-- Title -->
                            <h2 class="leading-none text-sm text-justify"><?php echo $title;?></h2>
                            <ul class="text-xs flex flex-wrap gap-1 *:px-3 *:rounded-xl *:bg-[#1b3a1a90] *:text-xs *:font-extralight *:opacity-85 *:select-all">
                                <?php
                                    ...
                                    foreach ($authorsList as $author) {
                                        echo "<li>" . trim($author) . "</li>";
                                    }
                                ?>
                            </ul>
                        </div>
                    </div>
                    <div class="fixed top-0 right-0 m-5 w-[calc(100vw-120px)] min-h-[calc(100vh-40px)] h-auto rounded-4xl bg-[#eeeeee] z-5 text-black hidden full-view"> <!-- FULL VIEW -->
                        <div class="absolute top-1/2 right-0 -translate-y-1/2 p-15 w-full h-[60vh] grid grid-cols-[25%_75%] bg-[#355e3b]">
                            <div class="relative w-auto h-full">
                                <img src="resources/book.svg" alt="big-book" onclick="toggleFullView(<?php echo $thesis_id;?>)" class="absolute hover:-translate-y-4 duration-200 drop-shadow-2xl select-none cursor-zoom-out" draggable="false">
                            </div>
                            <div class="ml-15 flex flex-col text-justify text-gray-300 font-bold">
                                <!-- Title -->
                                <h2 class="text-xl leading-5"><?php echo $title;?></h2>
                                <p class="mt-1 select-none opacity-80"><?php echo $published_date?></p> <!--Published Date -->
                            ...
                            <button class="absolute top-5 right-5 p-2 rounded-2xl bg-[#1b3a1a90] hover:opacity-80 active:scale-95 duration-100 cursor-pointer" onclick="toggleFullView(<?php echo $thesis_id;?>)">
                            ...
                            <div class="absolute bottom-0 right-0 pb-5 pr-15 flex gap-2 *:bg-[#1b3a1a90] *:px-4 *:py-0.5 *:rounded-xl *:flex *:gap-1 *:items-center *:hover:opacity-80 *:active:scale-95 *:duration-100 *:select-none *:cursor-pointer text-gray-300 ">
                                <button onclick="copyCitation(<?php echo $thesis_id;?>)">
                                ...
                                <button onclick="bookmark(<?php echo $thesis_id;?>)">
    ...
    <script>
        const libItems = document.querySelectorAll(".libitems");

        function toggleFullView(id) {
            document.getElementById(id).querySelector('.full-view').classList.toggle('hidden');
            /*
            libItems.forEach(item => {
                item.classList.toggle('h-45');
            });
            */
        }
    </script>
